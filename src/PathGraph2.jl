...
import ..DEFAULT_MISSING
import ..AUTO
import ..PoolArrayOptions
...
function LeafNode(name_path)
    ...
    return PathNode'.LeafNode(default_value, pool_arrays, dtype)
end
...
function get_children(node::PathNode)
    return @cases node begin
        ...
        LeafNode(_, _, _) => nothing
    end
end

function get_default_value(node::PathNode)
    return @cases node begin
        LeafNode(_, default_value, _) => default_value
...
function get_pool_arrays(node::PathNode)
    return @cases node begin
        LeafNode(_, _, pool_arrays) => pool_arrays
